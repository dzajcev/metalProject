...
call liquibase --driver=org.postgresql.Driver --username=%user% --liquibaseSchemaName=public --changeLogFile=initial_mt.xml --url=%url%/postgres?tcpKeepAlive:true --password=%pass% --logLevel=info update  > %sql_name%
IF NOT %ERRORLEVEL%  == 0 (CALL :logger ***ERROR***
GOTO endscr)

echo %DATE% %TIME% mtdb prerun changesets
echo %DATE% %TIME% mtdb prerun changesets >> %log_name%
call liquibase --driver=org.postgresql.Driver --username=%user% --liquibaseSchemaName=public --changeLogFile=prerun_mt.xml --url=%url%/mtdb?tcpKeepAlive:true --password=%pass% --logLevel=info update  >> %sql_name%
IF NOT %ERRORLEVEL%  == 0 (CALL :logger ***ERROR***
GOTO endscr)

echo %DATE% %TIME% mtdb master changesets
echo %DATE% %TIME% mtdb master changesets >> %log_name%
call liquibase --driver=org.postgresql.Driver --logLevel=info --username=%user% --liquibaseSchemaName=public --changeLogFile=master_mt.xml --url=%url%/mtdb?tcpKeepAlive:true --password=%pass% update  >> %sql_name%
IF NOT %ERRORLEVEL%  == 0 (CALL :logger ***ERROR***
GOTO endscr)

echo %DATE% %TIME% mtdb postrun changesets
echo %DATE% %TIME% mtdb postrun changesets >> %log_name%
call liquibase --driver=org.postgresql.Driver --username=%user% --liquibaseSchemaName=public --changeLogFile=postrun_mt.xml --url=%url%/mtdb?tcpKeepAlive:true --password=%pass% --logLevel=info update  >> %sql_name%
IF NOT %ERRORLEVEL%  == 0 (CALL :logger ***ERROR***
GOTO endscr)
...
:endscr
echo %DATE% %TIME% METAL mtdb UPDATE end
echo %DATE% %TIME% METAL mtdb UPDATE end >> %log_name%
echo  >> %log_name%
goto end
