...
public final class AsymmetricCipher {
    ...
    public CryptoPacket encrypt(byte[] data, String privateKeyBase64) throws CryptoException {
        ...
        final PublicKey publicKey = rsaGenerator.getPublicKeyFromBase64String(privateKeyBase64);
     ...
     * @throws CryptoException if the data could not be encrypted.
     */
    public CryptoPacket encrypt(byte[] data, PublicKey publicKey) throws CryptoException {
     ...
     */
    private byte[] decrypt(CryptoPacket cryptoPacket, String publicKeyBase64) throws CryptoException {
        // Decrypt cryptoPacket#encryptedSymmetricKey using asymmetricKey
        final RSAKeyPairGenerator rsaGenerator = new RSAKeyPairGenerator();
        final PrivateKey privateKey = rsaGenerator.getPrivateKeyFromBase64String(publicKeyBase64);
        return decrypt(cryptoPacket, privateKey);
    }
     ...
     * @return byte array of the decrypted data.
     * @throws CryptoException if the privateKey cannot decrypt the encrypted symmetric key, or the symmetric key cannot decrypt the data.
     */
